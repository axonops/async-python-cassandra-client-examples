...
async def lifespan(app: FastAPI):
    # Startup
    logger.info("Starting sync FastAPI application")
    db.connect()
    yield
    # Shutdown
    logger.info("Shutting down sync FastAPI application")
    db.close()


app = FastAPI(
    title="Sync Cassandra Performance Test",
    description="FastAPI application using standard cassandra-driver",
    version="1.0.0",
    lifespan=lifespan
)
