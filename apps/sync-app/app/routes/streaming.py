...
def sensor_data_generator(
    ...
    try:
        # Create a statement with fetch size for streaming
        statement = SimpleStatement(query, fetch_size=batch_size)
        result = session.execute(statement, params if params else None)
        ...
        for row in result:
            ...
            if len(buffer) >= batch_size:
                ...
                yield "\n".join(buffer) + "\n"
                buffer = []
                time.sleep(0.01)  # Small delay to prevent overwhelming client
...
async def stream_documents(
    ...
    def generate():
        ...
        if title_filter:
            query += " WHERE title = %s ALLOW FILTERING"
